((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_61",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,B={aOW:function aOW(d,e,f){this.c=d
this.d=e
this.a=f},cwq:function cwq(d,e){this.a=d
this.b=e},cwr:function cwr(d,e){this.a=d
this.b=e}},D
A=c[0]
C=c[2]
B=a.updateHolder(c[149],B)
D=c[293]
B.aOW.prototype={
m(d){var x,w,v,u,t,s,r,q=this,p=null,o=$.ad.n(),n=A.bm(p,p,p,p,p,p,p,p,p,p,p,12,p,p,p,p,p,!0,p,p,p,p,p,p,p,p)
if(q.d)x="recommendedToUpdateBeforePurchace"
else x=q.c?"recommendedToUpdateBeforeChange":"recommendedToUpdate"
x=A.k(x,p,p,!1)
w=A.p(d).p2.z
w.toString
v=A.p(d).ax
u=v.T
v=u==null?v.k3:u
u=C.f.W(178.5)
x=A.bB(p,p,w.cb(A.Q(u,v.gi(v)>>>16&255,v.gi(v)>>>8&255,v.gi(v)&255),12),x)
v=A.k("policy",p,p,!0)
w=A.pn(p,p)
w.bH=new B.cwq(q,d)
t=A.p(d).p2.z
t.toString
v=A.bB(p,w,t.cb(A.p(d).ax.y,12),v)
t=A.k("andIn",p,p,!1)
w=A.p(d).p2.z
w.toString
s=A.p(d).ax
r=s.T
s=r==null?s.k3:r
t=A.bB(p,p,w.cb(A.Q(u,s.gi(s)>>>16&255,s.gi(s)>>>8&255,s.gi(s)&255),12)," "+t)
s=A.k("termToUse",p,p,!0)
u=A.pn(p,p)
u.bH=new B.cwr(q,d)
w=A.p(d).p2.z
w.toString
return new A.P(D.aQU,new A.l(o*0.7,p,A.oM(p,p,C.aT,p,p,!0,p,A.bB(A.a([x,v,t,A.bB(p,u,w.cb(A.p(d).ax.y,12),s)],y.a),p,n,p),C.o,p,p,C.a0,C.a_),p),p)},
za(d){return this.chX(d)},
chX(d){var x=0,w=A.i(y.f)
var $async$za=A.d(function(e,f){if(e===1)return A.e(f,w)
while(true)switch(x){case 0:x=2
return A.b(A.pw(d,!0,"https://simpletor.app/privacy",!0),$async$za)
case 2:return A.f(null,w)}})
return A.h($async$za,w)},
zb(d){return this.ci6(d)},
ci6(d){var x=0,w=A.i(y.f)
var $async$zb=A.d(function(e,f){if(e===1)return A.e(f,w)
while(true)switch(x){case 0:x=2
return A.b(A.pw(d,!0,"https://simpletor.app/terms-of-use",!0),$async$zb)
case 2:return A.f(null,w)}})
return A.h($async$zb,w)}}
var z=a.updateTypes([])
B.cwq.prototype={
$0(){return this.a.za(this.b)},
$S:0}
B.cwr.prototype={
$0(){return this.a.zb(this.b)},
$S:0};(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.aOW,A.t)
w(A.bs,[B.cwq,B.cwr])})()
A.bp(b.typeUniverse,JSON.parse('{"aOW":{"t":[],"j":[]}}'))
var y={a:A.B("H<wE>"),f:A.B("~")};(function constants(){D.aQU=new A.w(0,12,0,0)})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_61",e:"endPart",h:b})})($__dart_deferred_initializers__,"HhU2qa35MEqUPEL2fUl7WzwPa6Y=");