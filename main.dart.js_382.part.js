((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_382",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,B={
fmc(d,e,f,g,h,i){return new B.b5Y(d,i,g,h,e,f,null)},
b5Y:function b5Y(d,e,f,g,h,i,j){var _=this
_.c=d
_.d=e
_.e=f
_.f=g
_.r=h
_.w=i
_.a=j},
b5U:function b5U(d,e,f,g){var _=this
_.c=d
_.d=e
_.e=f
_.a=g},
cv_:function cv_(d,e){this.a=d
this.b=e},
cv0:function cv0(d,e){this.a=d
this.b=e},
b5V:function b5V(d,e,f,g){var _=this
_.c=d
_.d=e
_.e=f
_.a=g},
cv1:function cv1(d,e){this.a=d
this.b=e},
cv2:function cv2(d,e){this.a=d
this.b=e}}
A=c[0]
C=c[2]
B=a.updateHolder(c[283],B)
B.b5Y.prototype={
m(d){var x,w,v,u,t,s,r,q,p=this,o=null
A.bJ(d,!0,y.j)
x=!p.f
w=x?new A.m(o,o,o,o):new B.b5U(p.e,p.d,p.w,o)
x=x?new A.m(o,o,o,o):new B.b5V(p.e,p.d,p.w,o)
v=p.w
if(v)u=A.t(d).ax.k2
else{u=A.t(d).ax
t=u.CW
u=t==null?u.y:t}t=A.ru(new A.L(Date.now(),0,!1),o,o,!0,!0)
t=A.a0(p.c,"OLDDATE",t)
s=A.ru(A.cr(new A.L(Date.now(),0,!1),A.R(0,2,0,0,0,0)),o,o,!0,!0)
t=A.a0(t,"NEWDATE",s)
s=A.ru(new A.L(Date.now(),0,!1),o,o,!0,!0)
t=A.a0(t,"DATE",s)
s=p.d
r=s.fr
t=A.a0(t,"WORKERNAME",r)
r=A.k("example",o,o,!0)
t=A.a0(t,"TREATMENTNAME",r)
if(p.gbqa()){r=A.k("atAdrees",o,o,!0)
q=$.F().a.k1.a
r=A.a0(r,"ADRESS",q)+" "}else r=""
t=A.a0(t,"ADRESS",r)
if(p.gbqk()){r=A.k("toDetails",o,o,!0)
s=A.ho(s.b)
s=" "+A.a0(r,"PHONE",s)}else s=""
t=A.a0(t,"PHONE",s)
s=$.F().a.k4
t=A.a0(t,"BUSINESSNAME",s)
s=A.nI().ga2K()
t=A.a0(t,"LINK","https://simpletor.app/"+s+"...")
s=p.r
if(s==null)s=A.k("client",o,o,!0)
return A.G(A.a([w,x,A.a6(o,o,o,o,A.y(A.a0(t,"NAME",s),C.q,!1,!1,o,!1,!1,o,!1,!1,v,!v,1,o,!1,!1,!1,15,o,""),u,o,0,!1,o,o,o,new A.x(10,10,10,10),!1,o,o,new A.x(10,10,10,10),15,o,!1,!1,!1,o)],y.e),C.d,C.c,C.e)},
gbqk(){if(!this.f)return!1
var x=this.d.L
if(this.e!=null)return x.c
else return x.d},
gbqa(){if(!this.f)return!1
var x=this.d.L
if(this.e!=null)return x.a
else return x.b}}
B.b5U.prototype={
m(d){var x,w=this,v=null
$.F()
x=w.d.L
x=w.c!=null?x.a:x.b
return A.d1(new A.kM(A.aZ(v,!1,!1,!1,!1,C.H,v,!0,!1,C.k5,75,!1,!1,v,v,"adress",!1,!1,!1,!0,v,v,v,new B.cv_(w,d),w.e,v,0,v,!0,!1,v,v,!0,v,v,v,!0,!0,v,new A.dr(x,new B.cv0(w,d),v),v,!0,v,0.3),v,!1,v),1)},
abk(d){return this.c9y(d)},
c9y(d){var x=0,w=A.i(y.f),v=this,u,t,s,r
var $async$abk=A.d(function(e,f){if(e===1)return A.e(f,w)
while(true)switch(x){case 0:$.F()
u=v.d
t=u.L
if(v.c!=null){s=A.nu(t.M())
s.a=!A.nu(u.L.M()).a
r=s}else{s=A.nu(t.M())
s.b=!A.nu(u.L.M()).b
r=s}A.cZ()
$.br().xg(d,r,u)
return A.f(null,w)}})
return A.h($async$abk,w)}}
B.b5V.prototype={
m(d){var x=this,w=null,v=x.c==null,u=x.d.L
u=!v?u.c:u.d
return new A.kM(A.aZ(w,!1,!1,!1,!1,C.H,w,!0,!1,C.l0,74,!1,v,w,w,"phoneNumber",!1,!1,!1,!0,w,w,w,new B.cv1(x,d),x.e,w,0,w,!0,!1,w,w,!0,w,w,w,!0,!0,w,new A.dr(u,new B.cv2(x,d),w),w,!0,w,0.3),w,!1,w)},
abz(d){return this.c9S(d)},
c9S(d){var x=0,w=A.i(y.f),v=this,u,t,s,r
var $async$abz=A.d(function(e,f){if(e===1)return A.e(f,w)
while(true)switch(x){case 0:s=v.d
r=s.L
if(v.c!=null){u=A.nu(r.M())
u.c=!A.nu(s.L.M()).c
t=u}else{u=A.nu(r.M())
u.d=!A.nu(s.L.M()).d
t=u}A.cZ()
$.br().xg(d,t,s)
return A.f(null,w)}})
return A.h($async$abz,w)}}
var z=a.updateTypes([])
B.cv_.prototype={
$0(){return this.a.abk(this.b)},
$S:2}
B.cv0.prototype={
$2(d,e){return this.baD(d,e)},
baD(d,e){var x=0,w=A.i(y.f),v,u=this
var $async$$2=A.d(function(f,g){if(f===1)return A.e(g,w)
while(true)switch(x){case 0:x=3
return A.c(u.a.abk(u.b),$async$$2)
case 3:v=g
x=1
break
case 1:return A.f(v,w)}})
return A.h($async$$2,w)},
$S:21}
B.cv1.prototype={
$0(){return this.a.abz(this.b)},
$S:2}
B.cv2.prototype={
$2(d,e){return this.baE(d,e)},
baE(d,e){var x=0,w=A.i(y.f),v,u=this
var $async$$2=A.d(function(f,g){if(f===1)return A.e(g,w)
while(true)switch(x){case 0:x=3
return A.c(u.a.abz(u.b),$async$$2)
case 3:v=g
x=1
break
case 1:return A.f(v,w)}})
return A.h($async$$2,w)},
$S:21};(function inheritance(){var x=a.inheritMany
x(A.r,[B.b5Y,B.b5U,B.b5V])
x(A.b7,[B.cv_,B.cv1])
x(A.ca,[B.cv0,B.cv2])})()
A.aY(b.typeUniverse,JSON.parse('{"b5Y":{"r":[],"l":[]},"b5U":{"r":[],"l":[]},"b5V":{"r":[],"l":[]}}'))
var y={e:A.z("I<l>"),j:A.z("iw"),f:A.z("~")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_382",e:"endPart",h:b})})($__dart_deferred_initializers__,"SM88C7HU1dwE7mvtx1ZWwX23Pe8=");