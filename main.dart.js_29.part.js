((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_29",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,B,C={
aSy(d,e,f){var x=0,w=A.i(y.f),v,u,t,s,r
var $async$aSy=A.d(function(g,h){if(g===1)return A.e(h,w)
while(true)switch(x){case 0:r=y.h.a(d.gaP())
$.aV().qU(0,f)
v=f.bV(0,"\n")
u=A.f5(r.dJ(0,null),B.w)
t=r.gD(0)
s=u.a
u=u.b
x=2
return A.c(A.dcB(e+"\n"+v,new A.a8(s,u,s+t.a,u+t.b),null),$async$aSy)
case 2:return A.f(null,w)}})
return A.h($async$aSy,w)}}
A=c[0]
B=c[2]
C=a.updateHolder(c[228],C)
var z=a.updateTypes([])
var y={h:A.E("ac?"),f:A.E("~")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_29",e:"endPart",h:b})})($__dart_deferred_initializers__,"fQTeDisJ3vlccPe2qOCn/8coJUU=");