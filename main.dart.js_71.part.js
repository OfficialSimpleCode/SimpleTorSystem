((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_71",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,B={blN:function blN(d,e,f){this.c=d
this.d=e
this.a=f},dkh:function dkh(d,e){this.a=d
this.b=e},dki:function dki(d,e){this.a=d
this.b=e}},D
A=c[0]
C=c[2]
B=a.updateHolder(c[196],B)
D=c[399]
B.blN.prototype={
m(d){var x,w,v,u,t,s,r,q=null,p=$.a9.n(),o=A.bd(q,q,q,q,q,q,q,q,q,q,q,12,q,q,q,q,q,!0,q,q,q,q,q,q,q,q)
if(this.d)x="recommendedToUpdateBeforePurchace"
else x="recommendedToUpdate"
x=A.l(x,q,q,!1)
w=A.r(d).p2.z
w.toString
v=A.r(d).ax
u=v.a7
v=u==null?v.k3:u
u=C.i.U(178.5)
x=A.bG(q,q,w.c1(A.U(u,v.gj(v)>>>16&255,v.gj(v)>>>8&255,v.gj(v)&255),12),x)
v=A.l("policy",q,q,!0)
w=A.p4(q,q)
w.bM=new B.dkh(this,d)
t=A.r(d).p2.z
t.toString
v=A.bG(q,w,t.c1(A.r(d).ax.y,12),v)
t=A.l("andIn",q,q,!1)
w=A.r(d).p2.z
w.toString
s=A.r(d).ax
r=s.a7
s=r==null?s.k3:r
t=A.bG(q,q,w.c1(A.U(u,s.gj(s)>>>16&255,s.gj(s)>>>8&255,s.gj(s)&255),12)," "+t)
s=A.l("termToUse",q,q,!0)
u=A.p4(q,q)
u.bM=new B.dki(this,d)
w=A.r(d).p2.z
w.toString
return new A.O(D.aTt,new A.m(p*0.7,q,A.oY(q,q,q,C.bd,q,q,!0,q,A.bG(A.a([x,v,t,A.bG(q,u,w.c1(A.r(d).ax.y,12),s)],y.a),q,o,q),C.p,q,q,C.ah,C.ai),q),q)},
Bb(d){return this.cAe(d)},
cAe(d){var x=0,w=A.i(y.f)
var $async$Bb=A.d(function(e,f){if(e===1)return A.e(f,w)
while(true)switch(x){case 0:x=2
return A.c(A.S8(d,!0,"https://simpletor.app/privacy",!0),$async$Bb)
case 2:return A.f(null,w)}})
return A.h($async$Bb,w)},
Bc(d){return this.cAn(d)},
cAn(d){var x=0,w=A.i(y.f)
var $async$Bc=A.d(function(e,f){if(e===1)return A.e(f,w)
while(true)switch(x){case 0:x=2
return A.c(A.S8(d,!0,"https://simpletor.app/terms-of-use",!0),$async$Bc)
case 2:return A.f(null,w)}})
return A.h($async$Bc,w)}}
var z=a.updateTypes([])
B.dkh.prototype={
$0(){return this.a.Bb(this.b)},
$S:1}
B.dki.prototype={
$0(){return this.a.Bc(this.b)},
$S:1};(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.blN,A.t)
w(A.bj,[B.dkh,B.dki])})()
A.b6(b.typeUniverse,JSON.parse('{"blN":{"t":[],"k":[]}}'))
var y={a:A.B("K<Xu>"),f:A.B("~")};(function constants(){D.aTt=new A.x(0,12,0,0)})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_71",e:"endPart",h:b})})($__dart_deferred_initializers__,"asY0XxpydwenrU6EzSxVDKw2Sao=");